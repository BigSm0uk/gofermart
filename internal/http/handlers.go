...
// Handler представляет HTTP хендлеры
type Handler struct {
...
func NewHandler(userService *service.UserService, orderService *service.OrderService, loyaltyService *service.LoyaltyService) *Handler {
	return &Handler{
...
func (h *Handler) WithdrawFunds(c fiber.Ctx) error {
	userID := c.Locals("userID").(uuid.UUID)
...
func (h *Handler) GetUserWithdrawals(c fiber.Ctx) error {
	userID := c.Locals("userID").(uuid.UUID)
